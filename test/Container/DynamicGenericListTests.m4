module DynamicGenericListTests
  
  use TestUtilities
  use Global
  use DynamicGenericModule
  
  
  implicit none
  
  character(*), parameter :: MOD_NAME = 'DynamicGenericListTests'
...
contains
  ...
  subroutine testDeinit
    class(GenericInterface), pointer :: ptr
  ...
  subroutine testExtractFirst
    ...
    call iterator%first()
    call iterator%extract( gen, log )
    call destroy( gen, log )
...
end module DynamicGenericListTests
